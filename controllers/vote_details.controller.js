...
exports.index = async (req, res) => {
	try {
		const response = await vote_details.findAll({ attributes: ['voteId','date','userId','agree'],include:[{model:votes}] });
...
exports.store = async (req, res) => {
	try {
		...
		data.forEach(element => {
			response.push(vote_details.create(element, { fields: ['voteId', 'date','userId'] })) 
